...
const employeesBonus = (arr) => {
    ...
    function salaryConvert(salary, bonus) {
        let newSalary = parseInt(salary.replace(/\$/g, "")); // 100$ => 100
        let bonusSalary = newSalary + bonus; 
        return bonusSalary + "$";
    }

    function bonusEmployee(employee) {
        if (employee.workHours > 8) { 
            employee.salary = salaryConvert(employee.salary, 100); 
        } else {
            employee.salary = salaryConvert(employee.salary, 50);
        }
    }
    arr.forEach(bonusEmployee);
    return arr;
}     
// -------------------------------------------------------------------------------------------------------
